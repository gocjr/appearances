...
<div class="card shadow py-0">
    ...
    <div class="card-body p-0">
        <table class="table table-striped p-0 m-0" width="100%" style="width: 100%;">
            <tread>
                <tr>
                    <th style="width:10%"><?= $this->Form->checkbox('checkall') ?></th>
                    <?= $this->fetch('thread'); ?>
                </tr>
            </tread>
            <tbody>
                <?php foreach ($rows as $i => $row) : ?>
                    ...
                    ?>
                    <tr>
                        <td style="width:10%"><?= $this->Form->checkbox('check.' . $i, ['value' => $row->id, 'class' => 'child']) ?></td>
                        <?php eval('?>' . str_replace(['{{', '}}'], ['<?=', '?>'], $tbody) . '<?php'); ?>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
